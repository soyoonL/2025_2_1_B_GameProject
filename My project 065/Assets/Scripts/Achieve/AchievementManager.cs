...
public class AchievementManager : MonoBehaviour
{
    ...
    public void UpdateAchievementUI() // 업적 UI 업데이트 함수
    {
        if (achievementListContent == null || achievementSlotPrefab == null) return;

        foreach (Transform child in achievementListContent) // 기존 슬롯 제거
        {
    ...
    void ShowAchievementPopup(AchievementData achievement) // 업적 팝업 표시
    {
        if (achievementPopupPrefab != null && popupParent != null)
        {
            ...
            if (descText != null) descText.text = achievement.achievmentName;
